...
export async function updateUserRoleAction(
    ...
    if (!Object.values(UserRole).includes(newRole)) return { error: "Invalid role value provided." };
    ...
    try {
        ...
        console.log(`Admin ${session.user.email} updated role for user ${userId} to ${newRole}`);
...
export async function deleteUserAction(
    ...
    try {
        ...
        if (orderCount > 0) {
            return { error: `Cannot delete user: User has ${orderCount} associated order(s). Consider deactivating instead.` };
        }
        ...
        if (error.code === 'P2014' || error.code === 'P2003') {
             return { error: "Cannot delete user due to existing related records (e.g., orders)." };
        }
