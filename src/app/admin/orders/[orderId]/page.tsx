        ...
        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="border rounded-lg p-6">
                        <div className="flex flex-wrap justify-between items-start gap-4 mb-4">
                            <div>
                                <h1 className="text-xl font-bold mb-1">Order #{order.id.substring(0, 8)}...</h1>
                        ...
                        <p className="text-sm text-muted-foreground font-mono">{order.id}</p>
                         ...
                         {order.stripePaymentIntentId && (
                            <p className="text-xs text-muted-foreground mt-1">
                                Payment Intent: <span className="font-mono">{order.stripePaymentIntentId}</span>
                            </p>
                         )}
                    </div>
                    ...
                    {/* Order Items */}
                     <div className="border rounded-lg p-6">
                        <h2 className="text-lg font-semibold mb-4">Items Ordered ({order.items.length})</h2>
                        <div className="space-y-4">
                            {order.items.map((item) => (
                                <div key={item.id} className="flex items-start gap-4">
                                    <Link href={`/products/${item.product.slug}`} target="_blank"> {/* Open product in new tab */}
                                        <Image
                                    ...
                                    <div className="flex-grow">
                                        <Link href={`/products/${item.product.slug}`} target="_blank" className="font-medium hover:underline">
                                            {item.product.name}
                                        </Link>
                        ...
                        <div className="space-y-2 text-sm">
                            ...
                            <div className="flex justify-between font-semibold text-base"><span >Order Total</span><span>{formatPrice(total)}</span></div>
                        </div>
                     </div>
                </div>
                ...
                <div className="lg:col-span-1 space-y-6">
                    {/* Customer Info */}
                    <div className="border rounded-lg p-6">
                         <h2 className="text-lg font-semibold mb-4 flex items-center"><User className="mr-2 h-5 w-5"/>Customer</h2>
                    ...
                    <div className="border rounded-lg p-6 sticky top-20"> {/* Sticky actions */}
