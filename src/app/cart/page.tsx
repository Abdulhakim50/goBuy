            ...
            {cartItems.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        {cartItems.map((item) => (
                            <div key={item.id} className="flex items-center gap-4 p-4 border rounded-lg">
                                <Link href={`/products/${item.product.slug}`}>
                                    <Image
                                        src={item.product.images?.[0] ?? '/placeholder-image.png'} // Fallback image
                    ...
                    <div className="md:col-span-1">
                        <div className="p-6 border rounded-lg sticky top-20"> {/* Sticky summary */}
                            ...
                            <div className="flex justify-between font-bold text-lg mb-6">
                                <span>Total</span>
                                <span>{formatPrice(subtotal)}</span> {/* Update when taxes/shipping added */}
                            </div>
